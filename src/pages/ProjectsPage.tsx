...
const ProjectsPage: React.FC = () => {
  ...
  return (
    <MainLayout
      title="Projects"
      description="Proyek-proyek yang telah saya kerjakan."
      icon={<Code2 />}
    ...
    >
      <div className="border-4 border-black bg-zinc-800 rounded-2xl p-6 text-center">
        {loading ? (
          ...
          <div className="space-y-6">
            {posts.map(post => (
              <div key={post.id} className="border-4 border-black bg-zinc-800 text-white overflow-hidden rounded-lg">
                <div className="bg-green-400 text-black border-b-4 border-black p-4">
                  <div className="flex justify-between items-center">
                    <h3 className="text-xl font-bold">{post.title}</h3>
                    <span className="bg-black text-green-400 px-2 py-1 text-xs rounded-md">
                ...
                <div className="p-4">
                  ...
                  <div className="mt-4 flex justify-end">
                    <Link 
                      to={`/post/${post.slug}`}
                      className="bg-green-400 text-black font-bold py-1 px-4 rounded-md border-2 border-black hover:bg-green-500 transition-colors"
                    >
                      Baca →
                    </Link>
                  </div>
          ...
          <p className="text-white text-xl py-10">No Project posts found. Check back later!</p>
        )}
