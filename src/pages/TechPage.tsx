...
const TechPage: React.FC = () => {
  ...
  return (
    <MainLayout
      title="Tech Diarist"
      description="Review gadget, aplikasi, dan teknologi terkini. Catatan harian dan pengalaman dengan berbagai tools dan platform."
      icon={<NotebookPen />}
    ...
    >
      <div className="border-4 border-black bg-zinc-800 rounded-2xl p-6">
        {loading ? (
          ...
          <div className="space-y-6">
            {posts.map(post => (
              <div key={post.id} className="border-4 border-black bg-zinc-800 text-white overflow-hidden rounded-lg">
                <div className="bg-pink-400 text-black border-b-4 border-black p-4">
                  <div className="flex justify-between items-center">
                    <h3 className="text-xl font-bold text-left">{post.title}</h3>
                    <span className="bg-black text-pink-400 px-2 py-1 text-xs rounded-md">
                ...
                <div className="p-4 flex flex-col md:flex-row gap-4">
                  ...
                  <div className={`${post.featured_image ? 'md:w-3/4' : 'w-full'} text-left`}>
                    ...
                    <div className="mt-4 flex justify-start">
                      <Link 
                        to={`/post/${post.slug}`}
                        className="bg-pink-400 text-black font-bold py-1 px-4 rounded-md border-2 border-black hover:bg-pink-500 transition-colors"
                      >
                        Baca →
                      </Link>
                    </div>
          ...
          <p className="text-white text-xl py-10">No Tech posts found. Check back later!</p>
        )}
